...
const DEFAULTS = {
  ...
  permissionProfileFile: process.env.AGENT_PERMISSION_PROFILE_FILE || "",
  permissionProfileName: process.env.AGENT_PERMISSION_PROFILE_NAME || "",
...
Usage:
  ...
  node scripts/local-coding-agent.mjs permissions init|list|show|create|add-root|remove-root|use

Common options:
  --workspace <path>          Workspace root the agent may access
  --extra-roots <paths>       Extra roots, semicolon-separated
  --permission-profile-file <path>  Preview multi-root profile store (outside workspace)
  --permission-profile-name <name>  Named profile to activate
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    switch (arg) {
      ...
      case "--extra-roots":
        ...
        break;
      case "--permission-profile-file":
        flags.permissionProfileFile = next();
        break;
      case "--permission-profile-name":
      case "--name":
        flags.permissionProfileName = next();
        break;
      case "--preset":
        flags.preset = next();
        break;
...
function normalize(opts) {
  ...
  out.permissionProfileFile = out.permissionProfileFile ? resolve(out.permissionProfileFile) : "";
  out.permissionProfileName = String(out.permissionProfileName || "");
...
function validate(opts, { requireWorkspace = false, requireTunnel = false } = {}) {
  ...
  if (opts.permissionProfileFile && !existsSync(opts.permissionProfileFile)) {
    throw new Error(`Permission profile file does not exist: ${opts.permissionProfileFile}. Run permissions init first.`);
  }
...
function configId(opts) {
  const material = JSON.stringify({
    ...
    permissionProfileFile: opts.permissionProfileFile || "",
    permissionProfileName: opts.permissionProfileName || "",
...
// Internal local task manager CLI. Imports the same module the server uses so
// CLI-created and MCP-created local tasks share one local store.
...
const PERMISSION_PRESETS = new Set(["observe", "edit", "develop", "full_control"]);
...
function pathInside(target, root) {
  const child = comparablePath(target);
  const base = comparablePath(root);
  return child === base || child.startsWith(base.endsWith("\\") || base.endsWith("/") ? base : `${base}${process.platform === "win32" ? "\\" : "/"}`);
}

function permissionStorePath(flags = {}) {
  const cfg = loadConfig();
  return resolve(flags.permissionProfileFile || cfg.permissionProfileFile || DEFAULT_PERMISSION_PROFILE_PATH);
}

function readPermissionStore(file) {
  const store = readJsonFile(file, null);
  if (!store || typeof store !== "object" || !store.profiles || typeof store.profiles !== "object") {
    throw new Error(`Invalid permission profile store: ${file}`);
  }
  return store;
}

async function savePermissionStore(file, store) {
  mkdirSync(dirname(file), { recursive: true });
  await writeFile(file, `${JSON.stringify(store, null, 2)}\n`, "utf8");
  try { await chmod(file, 0o600); } catch { /* Windows may ignore POSIX mode. */ }
}

function assertPermissionStoreOutsideRoots(file, profiles) {
  const roots = Object.values(profiles || {}).flatMap((profile) => Array.isArray(profile?.roots) ? profile.roots : []);
  for (const root of roots) {
    const rootPath = typeof root === "string" ? root : root?.path;
    if (rootPath && pathInside(file, rootPath)) {
      throw new Error(`Permission profile store must stay outside authorized roots: ${file}`);
    }
  }
}

function normalizePermissionPreset(value, fallback = "develop") {
  const preset = String(value || fallback).toLowerCase();
  if (!PERMISSION_PRESETS.has(preset)) throw new Error(`Unknown preset "${value}". Use observe, edit, develop, or full_control.`);
  return preset;
}

async function permissionsCommand(rest, flags) {
  const [sub = "list", ...args] = rest;
  const file = permissionStorePath(flags);
  if (sub === "path") {
    console.log(file);
    return;
  }
  ...
  if (sub === "init") {
    const opts = effectiveOptions(flags);
    const workspace = resolve(opts.workspace || REPO_ROOT);
    if (!existsSync(workspace)) throw new Error(`Workspace does not exist: ${workspace}`);
    const name = String(flags.permissionProfileName || args[0] || "default");
    const preset = normalizePermissionPreset(flags.preset, opts.mode === "full" ? "full_control" : "develop");
    const store = {
      version: 1,
      active_profile: name,
      profiles: {
    ...
    assertPermissionStoreOutsideRoots(file, store.profiles);
    await savePermissionStore(file, store);
    const cfg = loadConfig();
    cfg.workspace = workspace;
    cfg.permissionProfileFile = file;
    cfg.permissionProfileName = name;
    await saveConfig(cfg);
    console.log(JSON.stringify({ ok: true, file, active_profile: name, workspace, preset }, null, 2));
    return;
  }

  const store = readPermissionStore(file);
  if (sub === "list") {
    const profiles = Object.values(store.profiles).map((profile) => ({
      name: profile.name,
      active: profile.name === store.active_profile,
      working_directory: profile.working_directory,
      roots: Array.isArray(profile.roots) ? profile.roots.length : 0
    }));
    console.log(JSON.stringify({ file, active_profile: store.active_profile, profiles }, null, 2));
    return;
  }

  if (sub === "show") {
    const name = String(args[0] || flags.permissionProfileName || store.active_profile);
    if (!store.profiles[name]) throw new Error(`Unknown permission profile "${name}".`);
    console.log(JSON.stringify({ file, active: name === store.active_profile, profile: store.profiles[name] }, null, 2));
    return;
  }

  if (sub === "create") {
    const name = String(args[0] || flags.permissionProfileName || "").trim();
    if (!name) throw new Error("Usage: permissions create <name> --workspace <path> [--preset develop]");
    if (store.profiles[name]) throw new Error(`Permission profile "${name}" already exists.`);
    const opts = effectiveOptions(flags);
    const workspace = resolve(flags.workspace || opts.workspace || REPO_ROOT);
    if (!existsSync(workspace)) throw new Error(`Workspace does not exist: ${workspace}`);
    const preset = normalizePermissionPreset(flags.preset, "develop");
    store.profiles[name] = {
      version: 1,
      name,
      description: "Local Coding Agent v5 multi-root profile",
    ...
    assertPermissionStoreOutsideRoots(file, store.profiles);
    await savePermissionStore(file, store);
    console.log(JSON.stringify({ ok: true, created: name, workspace, preset }, null, 2));
    return;
  }

  if (sub === "add-root") {
    const name = String(args[0] || store.active_profile);
    const rootPath = args[1] ? resolve(args[1]) : "";
    const preset = normalizePermissionPreset(args[2] || flags.preset, "develop");
    const profile = store.profiles[name];
    if (!profile) throw new Error(`Unknown permission profile "${name}".`);
    if (!rootPath) throw new Error("Usage: permissions add-root <profile> <path> [preset] [--deny glob1,glob2]");
    if (!existsSync(rootPath)) throw new Error(`Root does not exist: ${rootPath}`);
    const deny = String(flags.deny || "").split(",").map((item) => item.trim()).filter(Boolean);
    const root = { label: args[3] || rootPath.split(/[\\/]/).filter(Boolean).at(-1) || rootPath, path: rootPath, preset, ...(deny.length ? { deny } : {}) };
    const roots = Array.isArray(profile.roots) ? profile.roots : [];
    const index = roots.findIndex((candidate) => comparablePath(typeof candidate === "string" ? candidate : candidate.path) === comparablePath(rootPath));
    if (index >= 0) roots[index] = root;
    else roots.push(root);
    profile.roots = roots;
    assertPermissionStoreOutsideRoots(file, store.profiles);
    await savePermissionStore(file, store);
    console.log(JSON.stringify({ ok: true, profile: name, root }, null, 2));
    return;
  }
  ...
  if (sub === "remove-root") {
    ...
    const profile = store.profiles[name];
    if (!profile) throw new Error(`Unknown permission profile "${name}".`);
    if (!rootPath) throw new Error("Usage: permissions remove-root <profile> <path>");
    const roots = (Array.isArray(profile.roots) ? profile.roots : []).filter((candidate) => comparablePath(typeof candidate === "string" ? candidate : candidate.path) !== comparablePath(rootPath));
    if (!roots.length) throw new Error("A permission profile must keep at least one root.");
    const workingAllowed = roots.some((candidate) => pathInside(profile.working_directory, typeof candidate === "string" ? candidate : candidate.path));
    if (!workingAllowed) throw new Error("Cannot remove the root that contains working_directory until another containing root is configured.");
    profile.roots = roots;
    await savePermissionStore(file, store);
    console.log(JSON.stringify({ ok: true, profile: name, removed: rootPath }, null, 2));
    return;
  }
  ...
  if (sub === "use") {
    ...
    await savePermissionStore(file, store);
    const cfg = loadConfig();
    cfg.workspace = resolve(store.profiles[name].working_directory);
    cfg.permissionProfileFile = file;
    cfg.permissionProfileName = name;
    await saveConfig(cfg);
    console.log(JSON.stringify({ ok: true, active_profile: name, file, restart_required: true }, null, 2));
    return;
  }

  throw new Error(`Unknown permissions command: ${sub}`);
}
...
async function main() {
  ...
  if (command === "permissions" || command === "permission") return permissionsCommand(rest, flags);
