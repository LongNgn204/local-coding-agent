# v5.0.0 — Local-first, multi-root workflow

> **Official stable release.** The v5 feature set is enabled by default.
> `AGENT_V5_PREVIEW` is retained only as a backwards-compatible switch; set it
> to `0` when temporary v4 compatibility behavior is required.

v5 includes the local task-agent manager documented in
...
the local Chrome Companion documented in
[`experiments/chrome-companion/README.md`](../experiments/chrome-companion/README.md).

v5 adds named multi-root permission profiles, per-path rights, deny
...
v5 lets the Windows tray's explicit shutdown opt-in authorize the
...
that in the DOM and re-render it. v5's goal is simple:
...
The official v5 feature set is enabled by default:

```bash
# macOS/Linux
AGENT_WORKSPACE=/path/repo npm --prefix server start
```
...
5. **Customer flow helpers:**
   ...
   prompts.
6. **Chrome Companion:** an unpacked Manifest V3 extension pairs with
   ...
   capability reporting, and redacted last-action status. Users load it
   manually through Chrome's developer mode and explicitly arm one tab.
...
- `AGENT_V5_PREVIEW=0` — temporarily disable v5 for compatibility. The legacy
  variable name is retained so preview.12 configurations keep working.
- `AGENT_BROWSER_PREVIEW=0` — keep the rest of v5 enabled but disable Chrome
  Companion. Chrome Companion is enabled by default with v5.
- With v5 enabled, chat-facing defaults are intentionally smaller:
...
- Full skill runtime execution (v5.0.0 ships skill discovery + manifests
...
Bộ tính năng v5 chính thức được bật mặc định:
...
6. **Chrome Companion:** extension Manifest V3 dạng load-unpacked ghép
   ...
   gọn. Người dùng load extension thủ công trong Chrome developer mode và chủ
   động arm đúng một tab.
...
- `AGENT_V5_PREVIEW=0` — tạm tắt v5 để tương thích. Tên biến cũ được giữ lại để
  cấu hình từ preview.12 tiếp tục hoạt động.
- `AGENT_BROWSER_PREVIEW=0` — vẫn bật các phần v5 khác nhưng tắt Chrome
  Companion. Chrome Companion được bật mặc định cùng v5.
- Khi bật v5, default trả dữ liệu vào chat sẽ nhỏ hơn có chủ ý:
...
- Thực thi skill runtime đầy đủ (v5.0.0 chỉ có khám phá skill +
